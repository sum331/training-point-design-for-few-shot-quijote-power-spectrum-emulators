...
![Figure 1. Workflow of the proposed Quijote training-point design. The final Quijote path supplies the truth spectra for training and validation, whereas the auxiliary KUN path supplies only the bias prior used by the design loop. The final 64-point training set contains 32 PPR cold-start points and 32 active additions.](data\paper_figures\main_results_20260701\numbered_gallery\figures\Fig01_method_overview.png)
...
The discrete support-point estimator is therefore also consistent. It is important to note that, when \(\tau_\lambda\) defines a finite center target region, conditional unbiasedness refers to the average under that finite standard-geometry target region. If \(Y_F\) varies continuously with barycentric coordinates, the conditional average approaches the point value under the exact simplex-center condition as the target-region radius shrinks. The continuous field \(\widehat B(x)\) obtained by interpolation is a reliability-weighted smoothing of these conditionally unbiased support-point estimates. It introduces approximation error from smoothing, boundaries, and sparse support, so we do not claim that \(\widehat B(x)\) is strictly unbiased at every continuous location.

This distinction is central to the interpretation of the method. \(B_{\mathrm{SG},F}(x)\) is a conditional bias prior under a specified design generator \(\Pi_N\) and a specified standard-geometry event \(A_{\mathrm{SG}}(x,D)=1\). It is not an intrinsic Quijote error field independent of sample geometry, and it is not a pointwise estimate of the final Quijote emulator error. Its role is to provide a geometry-normalized shape prior for cold-start sample allocation. The final scientific claim is therefore not that \(\widehat B(x)\) exactly reconstructs the Quijote validation error, but that using this conditional prior improves the Quijote emulator when tested under the fixed validation protocol.
...
![Figure 3. Bias-field-driven PPR cold-start design. The continuous bias prior is mapped to a potential surface; particle relaxation balances attraction toward high-bias regions, inter-particle repulsion, and boundary regularization to produce 32 initial Quijote design points.](data\paper_figures\main_results_20260701\numbered_gallery\figures\Fig03_ppr_cold_start.png)
...
![Figure 5. Main fixed-budget result. With the same residual-anchor PCA-GP training workflow and the same fixed validation set, the proposed 64-point design lowers the overall p68 relative error from 0.019040 for Sobol64 to 0.014901.](data\paper_figures\main_results_20260701\numbered_gallery\figures\Fig05_overall_p68_main_result.png)
...
![Figure 6. Scale-dependent p68 validation error. The curve compares the per-k-bin p68 relative error of Sobol64 and the proposed method across the fixed 475-bin Quijote k-grid.](data\paper_figures\main_results_20260701\numbered_gallery\figures\Fig06_p68_k_curve.png)
...
![Figure 7. Band-wise p68 comparison. The proposed design has little effect in the low-k band but reduces bulk error in the mid, focus-high, and tail bands, where the active score gives stronger variance and bias weights.](data\paper_figures\main_results_20260701\numbered_gallery\figures\Fig07_band_p68_results.png)
...
Figure 8 summarizes the completed mechanism ablation under the same fixed 256-point LHS validation set. At the cold-start budget, PPR32 gives \(p68=0.021251\), compared with \(0.022609\) for Sobol32. This corresponds to a \(6.01\%\) improvement and shows that the standard-geometry bias prior improves the initial 32-point design.

The 64-point controls separate this cold-start effect from the active-selection effect. Filling the remaining budget after PPR32 with neutral Sobol points gives \(p68=0.019128\), close to but slightly worse than Sobol64 \((0.019040)\). PPR32 followed by variance-only active selection gives \(p68=0.019304\), also worse than Sobol64. In contrast, PPR32 followed by bias-only active selection reaches \(p68=0.017783\), a \(6.60\%\) improvement over Sobol64. The full variance-bias score gives the best result, \(p68=0.014901\), improving over Sobol64 by \(21.74\%\).

The initial-condition control further clarifies the mechanism. Starting from Sobol32 and then applying the same variance-bias active-selection rule gives \(p68=0.019858\), worse than Sobol64. Therefore, the final gain is not produced by the active acquisition score alone. The result requires the combination of the PPR cold-start geometry and the subsequent variance-bias active additions.

These controls support a component-level interpretation. PPR improves the early sample geometry, the KUN bias proxy contributes useful information about difficult regions, and the variance term becomes most effective when combined with the bias term after the PPR initial state. The completed workflow is therefore a coupled design rather than an independent sum of two interchangeable modules.

![Figure 8. Mechanism ablation. Lower values are better; the Sobol64 reference is marked for comparison. The full method gives the lowest overall \(p68\). PPR improves the 32-point cold start, bias-only active selection helps, and variance-only or Sobol-seeded active controls do not reproduce the final gain.](data\paper_figures\main_results_20260701\numbered_gallery\figures\Fig08_component_ablation_single_column.png)
...
![Figure 9. Error-distribution diagnostic. The proposed design shifts the main body of the validation-error distribution downward and reduces p50, p68, p90, p95, and p98, while the maximum error remains a separate tail-risk issue.](data\paper_figures\main_results_20260701\numbered_gallery\figures\Fig09_error_distribution_diagnostic.png)
...
![Figure 10. Training-point distribution diagnostics. The figure compares the Sobol baseline, PPR cold-start points, and active additions in normalized parameter projections, showing how the proposed workflow modifies coverage without collapsing to a single high-bias region.](data\paper_figures\main_results_20260701\numbered_gallery\figures\Fig10_design_distribution.png)
...
![Figure 12. Support quality of the standard-geometry bias field. The panels summarize reference-grid coverage, accepted counts, and reliability of the usable support points used to interpolate the continuous bias prior.](data\paper_figures\main_results_20260701\numbered_gallery\figures\Fig12_bias_field_support_quality.png)
...
![Figure 13. Two-dimensional slices of the interpolated bias prior. The slices visualize spatial structure in the reliability-weighted bias field and provide qualitative support for using the prior as a cold-start density guide.](data\paper_figures\main_results_20260701\numbered_gallery\figures\Fig13_bias_field_2d_slices.png)
...
This boundary must be explicit. The KUN bias proxy is useful only if its error shape is sufficiently aligned with the regions where the Quijote emulator is difficult to learn. If the auxiliary spectrum generator responds differently from Quijote in some parameter regions, \(B_t(x)\) may guide selection into regions that do not help Quijote. The present evidence is therefore empirical and component based. PPR32 shifts the cold-start design toward higher standard-geometry bias-field quantiles and improves over Sobol32 at the same 32-point budget. Bias-only active selection after PPR32 improves over Sobol64 by \(6.60\%\), whereas variance-only active selection after PPR32 is worse than Sobol64. The full variance-bias score then gives the best result. These controls support the use of KUN as a difficult-region shape proxy, while leaving multi-seed and multi-budget verification as necessary future tests.
...
Second, the applicability of the KUN bias proxy remains an empirical assumption rather than a theorem about Quijote errors. KUN provides an auxiliary error shape under the same training-point distribution, not a direct observation of Quijote bias. The current evidence consists of the PPR-versus-Sobol32 bias-quantile diagnostic, the bias-only and variance-only active-selection ablations, and the final Quijote validation result. These results show that the proxy is useful in the present fixed-budget setting, but they do not prove universal agreement between KUN and Quijote residual responses. Future work should quantify this relationship over multiple seeds, budgets, and validation sets, and should test whether regions highlighted by KUN also correspond to Quijote validation errors before and after active selection.
